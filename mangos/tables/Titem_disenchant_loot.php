<?php
    ...
    if (isset($_POST['update']) OR isset($_POST['add']))
    {
        if (isset($_POST['add']))
        {
             ...
             * */
            $query = "SELECT * FROM `disenchant_loot_template` WHERE `item` = ".$_POST['item']." && `entry` = $id";
            ...
            {
                $query = "INSERT INTO `disenchant_loot_template` (`entry`,`item`) VALUES ($id,".$_POST['item'].")";
                $sql = mysql_query($query) or die (mysql_error());
            }
        }/** ADD **/
            ...
            if ( ! empty($del_item))
            {
                ...
                for ($i = 0; $i<=$deleted_items_total-1; $i++)
                {
                    if ($del_item[$i] > "" &&
                        $del_item[$i] != 'del_item')
                    {
                        $query = "DELETE FROM `disenchant_loot_template` WHERE `item` = ".$del_item[$i]." AND `entry` = $id";
                ...
                for($i=0;$i<=$total_items-1;$i++)
                {
                    foreach($userData as $field[$i] => $value)
                    {
                        if($value > "" &&
                           ...
                           $field[$i] != 'item')
                        {
                            $query = "UPDATE `disenchant_loot_template` SET `$field[$i]`=$value[$i] WHERE `item` = ".$_POST['item_count'][$i]." AND `entry` = $id";
                            $sql = mysql_query($query) or die("Bad query<br>$query<br/>".mysql_error());
...
?>
     ...
     <form method="post">
    <fieldset>
        <legend>Enter an item id that <span class="npcnames"><?php echo $item['name'];?></span> will disenchant to</legend>
            <table>
                <tr>
    ...
    <fieldset>
        <legend>Items that are created when <span class="npcnames"><?php echo $item['name'];?></span> is disenchanted</legend>
            ...
            <?php
                ...
                $sql = mysql_query("SELECT * FROM `disenchant_loot_template` WHERE `entry`=$id") or die(mysql_error());
                $switch = 1;
